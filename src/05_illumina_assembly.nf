...
workflow ILLUMINA_ASSEMBLER {
   ...
   // BAKTA(RENAME_SHOVIL_ASSEMBLY.out, bakta_db, genus, species) //process failing due to erro with nxf_trace positional urgument
    ...
    //RENAME_SHOVIL_ASSEMBLY(SHOVILL.out.normal) //adresses the renaming of fasta files from shovil
    KLEBORATE(RENAME_SHOVIL_ASSEMBLY.out)
