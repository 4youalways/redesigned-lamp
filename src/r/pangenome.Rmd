...
metadata$date <- str_replace_na(metadata$date, replacement = "Unknown") 
...
#all_amr <-
  ...
  theme_light() +
  labs(y = "Total number of AMR gene class",
       x = "Date",
       fill = "AMR gene class") +
    scale_y_break(c(250, 1000)) +
...
# Subset the phylogenetic tree to the outbreak clade
## extract the strain accessions for the outbreak clade
```{r}
# Show the putbreak clade to extract
ggtree(tree) +
  geom_cladelab(node = 195,
                label = "Outbreak Isolates",
                barcolour = "red",
                align = F) +
  geom_hilight(node=195, fill="#6699CC")

# get the tip labs
extract.clade(tree, 195) %>% ggtree()

outbreak_clade_ids <-
  get_taxa_name(ggtree(tree), node = 195)

outbreak_clade_ids
```


## extract the strain accessions for the outbreak clade and the neigbouring strains
``` {r}

# extract the clade inluding the neigbouring clace
ggtree(tree) + geom_cladelab(node = 196,
                             label = "Outbreak Isolates",
                             barcolour = "red",
                             align = F) +
  geom_hilight(node=196, fill="#6699CC")

# get the tiplabs
extract.clade(tree, 196) %>% ggtree()
extra_clade_ids <-
  get_taxa_name(ggtree(tree), node = 196)
